...
from flowboard.services import claude_cli, vision as vision_service
...
@pytest.mark.asyncio
async def test_describe_media_passes_cached_path_to_cli(monkeypatch, tmp_path):
    """Service must locate the cached file, then forward the absolute path
    as a CLI attachment with the brief-system-prompt."""
    ...
    async def stub_run(prompt, *, system_prompt=None, attachments=None, timeout=0):
...
@pytest.mark.asyncio
async def test_describe_media_propagates_cli_failure(monkeypatch, tmp_path):
    ...
    async def stub_run(*a, **k):
        raise claude_cli.ClaudeCliError("auth failed")
